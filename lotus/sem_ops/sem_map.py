...
async def sem_map_async(
    ...
    """
    Asynchronously maps a list of documents to a list of outputs using a model.
    ...
    raw_outputs = await model.generate_async(inputs)
...
@pd.api.extensions.register_dataframe_accessor("sem_map_async")
class SemMapAsyncDataframe:
    """Async DataFrame accessor for semantic map."""
    ...
    async def __call__(
        ...
        """
        Asynchronously applies semantic map over a dataframe.
        ...
        outputs, raw_outputs, explanations = await sem_map_async(
            df_txt,
            lotus.settings.lm,
            formatted_usr_instr,
            postprocessor=postprocessor,
